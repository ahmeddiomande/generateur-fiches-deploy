...
# Fonction de formatage de la stack technique
...
# Fonction pour générer la fiche de poste via OpenAI
...
# Fonction de détection du domaine simplifié
...
# Supprimer l'ancien fichier RPO s'il existe
...
# Nettoyer les noms de fichiers pour éviter les caractères invalides
def nettoyer_nom_fichier(nom):
    return nom.replace("/", "_").replace("\\", "_").replace(":", "_").replace("?", "_").replace("*", "_").replace("\"", "_").replace("<", "_").replace(">", "_").replace("|", "_").replace("\n", "_")

# Créer un fichier ZIP pour contenir tous les fichiers
...
with zipfile.ZipFile(zip_file, 'w', zipfile.ZIP_DEFLATED) as zipf:
    ...
    # Générer et ajouter les fiches de poste et les mails au ZIP
    for row in sheet_rpo.iter_rows(min_row=2, values_only=True):
        ...
        # Générer et ajouter la fiche de poste au ZIP
        ...
        # Générer et ajouter le mail type au ZIP
